...
impl Screen {
    ...
    fn render_ruler(&mut self, changes: &mut Vec<Change>, row: usize) -> Result<(), Error> {
        ...
        //   [offset]  lines [ start line ]-[ end line ]/[line count]  [loadin]
        //    6      8        lw           1 lw         1 lw         1      iw
    ...
    pub(crate) fn maybe_load_more(&mut self) {
        // Fetch 1 screen + config.buffer_lines.
        let desired_lines = self.position.top + self.position.height * 2 + self.config.buffer_lines;
        self.file.set_desired_lines(desired_lines);
    }
}
